...
# --- 新增：所有用戶的生成圖片基礎路徑 ---
GENERATED_IMAGES_BASE_DIR = "/home/user/lora-train/generated_images_output"
...
# 初始歡迎訊息 (您可以根據需要進行國際化)
initial_welcome_message_zh = "歡迎使用 AI 生圖助理！請輸入'menu'或'選單' 查看功能選單，或輸入 '英文'/'English' 切換語言。"
...
def run_sd_generation(user_id, prompt, model_name, reply_token):
    ...
    # 為每個用戶創建專屬的輸出子資料夾
    user_output_dir = os.path.join(GENERATED_IMAGES_BASE_DIR, user_id)
    os.makedirs(user_output_dir, exist_ok=True)
    logging.info(f"User {user_id} generated images will be saved to: {user_output_dir}")

    # 清理該用戶目錄下舊的生成圖片 (可選，但推薦，避免累積過多舊圖片)
    for f_name in os.listdir(user_output_dir):
        if f_name.lower().endswith(('.png', '.jpg', '.jpeg')):
            try:
                os.remove(os.path.join(user_output_dir, f_name))
                logging.info(f"Cleaned up old image: {f_name} from {user_output_dir}")
            except OSError as e:
                logging.warning(f"Failed to remove old image {f_name}: {e}")

    # 預設 generated_image_path，確保 finally 塊中總是有這個變數
    ...
    try:
        ...
        result = SD_API.txt2img(
            ...
            width=512,
            height=768,
            save_images=False, # 建議設為 False，避免 WebUI 內部重複保存，由我們手動保存
            override_settings={"sd_model_checkpoint": model_name} # 確保使用指定模型
        )
        
        if result and result.images:
            # 生成唯一的檔名並保存到用戶專屬資料夾
            timestamp = int(time.time())
            generated_image_filename = f"generated_image_{timestamp}.png"
            generated_image_path = os.path.join(user_output_dir, generated_image_filename)

            # 將 webuiapi 返回的第一張圖片保存到指定路徑
        ...
        if generated_image_path and os.path.exists(generated_image_path):
            try:
                ...
                logging.info(f"Cleaned up generated image file: {generated_image_path}")
            except OSError as e:
                logging.error(f"Error cleaning up generated image file: {e}")
...
if __name__ == "__main__":
    ...
    os.makedirs(GENERATED_IMAGES_BASE_DIR, exist_ok=True) # 確保新的基礎目錄存在
    
    print(f"已創建模型目錄：{MODEL_FOLDER}，請將 .safetensors 模型文件放入此目錄。")
    print(f"所有用戶生成的圖片將會保存在：{GENERATED_IMAGES_BASE_DIR} 下的個別用戶資料夾中。")
